...
//TESTING OUR 4 SCENARIOS AT THE BOTTOM
func main() {
	Scenario1()
	Scenario2()
	Scenario3()
...
func elevator(ID, currentFloor, requestedFloor int) Elevator {
	m := new(Elevator)
	m.ID = ID
	for i := currentFloor; i <= requestedFloor; i++ {
...
func column(ID, currentFloor, requestedFloor, nbElevator int) Column {
	m := new(Column)
	m.ID = ID
	...
	// knowing the range of floors for each column (currentFloor - requestedFloor)
	for i := 1; i <= nbElevator; i++ {
		m.ElevatorList = append(m.ElevatorList, elevator(i, currentFloor, requestedFloor))
	}
	// Creating CallButtons for Basements (at if clause) and then for floors (at else clause)
	// knowing the range of floors for each column (currentFloor - requestedFloor)
	if ID == 1 {
		for i := requestedFloor; i <= currentFloor; i++ {
...
func battery(nbColumns, totalNbFloors, nbBasements, numberOfElevatorsPerColumn int) Battery {
	m := new(Battery)
	nbFloors := totalNbFloors - nbBasements
	avgFloorPerColumn := int(math.Floor(float64(nbFloors / (nbColumns - 1))))
	currentFloor := 2
	requestedFloor := avgFloorPerColumn
	currentColumnID := 1
	for currentColumnID <= nbColumns {
		// instantiation of the 1st column for the basements
		if currentColumnID == 1 {
			m.ColumnList = append(m.ColumnList, column(1, -nbBasements, -1, numberOfElevatorsPerColumn))
		} else if currentColumnID < nbColumns {
			// instantiation of the other columns
			m.ColumnList = append(m.ColumnList, column(currentColumnID, currentFloor, requestedFloor, numberOfElevatorsPerColumn))
			currentFloor = requestedFloor + 1
			requestedFloor = requestedFloor + avgFloorPerColumn
		} else {
			// instantiation of the last columns
			m.ColumnList = append(m.ColumnList, column(currentColumnID, currentFloor, nbFloors, numberOfElevatorsPerColumn))
		}
		currentColumnID += 1
	}
...
func (c Battery) RequestElevator(FloorNumber int) {
	...
	if BestElevator.Direction == Direction || BestElevator.Direction == "IDLE" {
		...
		// Updating the ElevatorList t of the selected elevator
	} else {
		fmt.Printf("***444 Take ElevatorID: %d which is currently at floor %d. \n", BestElevator.ID, BestElevator.Position)
		BestElevator.ElevatorList = c.UpdateList(BestElevator, BestElevator.ElevatorList, FloorNumber)
		BestElevator.StopList = c.UpdateList(BestElevator, BestElevator.StopList, 1)
		fmt.Print(" StopList: ")
		for i := 0; i < len(BestElevator.StopList); i++ {
			fmt.Print(" | ", BestElevator.StopList[i])
		...
		fmt.Println("")
		fmt.Print(" StopList: ")
		for i := 0; i < len(BestElevator.ElevatorList); i++ {
			fmt.Print(" | ", BestElevator.ElevatorList[i])
		}
		fmt.Println("")
...
func (c Battery) AssignElevator(RequestedFloor int) {
	...
	if BestElevator.Position == 1 {
		...
		// fmt.Print("666 current ElevatorList BEFORE update: ")
		// for i := 0; i < len(BestElevator.StopList); i++ {
		// 	fmt.Print(" | ", BestElevator.StopList[i])
		// }
		// fmt.Println("")
		BestElevator.StopList = c.UpdateList(BestElevator, BestElevator.ElevatorList, RequestedFloor)
		// fmt.Print("666 current ElevatorList BEFORE update: ")
		// for i := 0; i < len(BestElevator.StopList); i++ {
		// 	fmt.Print(" | ", BestElevator.StopList[i])
		// }
...
func (c Battery) UpdateList(elevator Elevator, List []int, Position int) []int {
	...
	if check {
		...
		sort.Ints(List) //sorting the list
	}
	return List
}
func (c Battery) move(elevator Elevator) {
	...
	for i <= len(elevator.StopList) {
		...
		fmt.Println("Door is closing in 5...4...3...2...1")
		fmt.Println("Door closed")
		elevator.Direction = "IDLE"
	}
	if len(elevator.ElevatorList) > 0 {
		elevator.StopList = elevator.ElevatorList
